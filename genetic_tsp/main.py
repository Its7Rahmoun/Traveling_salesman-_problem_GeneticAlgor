...
class mymain():
    ...
    def getdateinisial(self):
        ...
        shortestpath = []
        with open(self.selecteditemfile, 'r') as od:
            for idd, i in enumerate(od):
                ligne = i.strip()
                ligne = ligne.split(" ")
                shortestpath.append(ligne[0])
            od.close()
        shortestpath1 = []
        for i in best_solution.chromosome:
            shortestpath1.append(shortestpath[i.id])
        shortestpathtext = ''
        for k in shortestpath1:
            shortestpathtext += '--'+str(k)

        shortest = tk.Label(
            self.root, text="shortest path is :"+shortestpathtext)
        shortest.pack()
    ...
    def filelocations(self, listbox):
        ...
        geolocator = Nominatim(user_agent="short path")
        index = 0
